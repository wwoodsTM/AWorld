...
class Agent(BaseAgent[Observation, List[ActionModel]]):
    ...
    async def run_hooks(self, context: Context, current_agent_context: AgentContext, hook_point: str) -> AsyncGenerator[
        Message, None]:
    ...
    async def _execute_tool(self, actions: List[ActionModel]) -> Any:
        ...
        for tool_name, action in tool_mapping.items():
            ...
            if info.get("exception"):
                color_log(f"Agent {self.name()} _execute_tool failed with exception: {info['exception']}",
                          color=Color.red)
...
class PreLLMCallContextProcessHook(PreLLMCallHook):
    ...
    async def exec(self, message: Message, current_agent_context: AgentContext = None,
        ...
        with trace.span(f"llm_context_pre_hook", attributes={
            ...
            try:
                ...
                logger.debug(
                    f'debug|llm_context_compress|{origin_len}|{compressed_len}|{origin_messages_count}|{truncated_messages_count}|\n|{origin_messages}\n|{messages}')
            finally:
