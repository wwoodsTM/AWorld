...
class LLMModel:
    ...
    def __init__(self, conf: Union[ConfigDict, AgentConfig] = None, custom_provider: LLMProviderBase = None, **kwargs):
        ...
        kwargs['client_type'] = conf.llm_client_type if conf and "llm_client_type" in conf else ClientType.SDK
...
def get_llm_model(conf: Union[ConfigDict, AgentConfig] = None, custom_provider: LLMProviderBase = None, **kwargs) -> Union[LLMModel, ChatOpenAI]:
    ...
    if (llm_provider == "chatopenai"):
        base_url = kwargs.get("base_url") or (conf.llm_base_url if conf and "llm_base_url" in conf else None)
        model_name = kwargs.get("model_name") or (conf.llm_model_name if conf and "llm_model_name" in conf else None)
        api_key = kwargs.get("api_key") or (conf.llm_api_key if conf and "llm_api_key" in conf else None)

        return ChatOpenAI(
            model=model_name,
            temperature=kwargs.get("temperature", conf.llm_temperature if conf and "llm_temperature" in conf else 0.0),
            base_url=base_url,
            api_key=api_key,
        )
